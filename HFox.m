function out = HFox(an, An, ap, Ap, bm, Bm ,bq, Bq,z)

%% Integrand definition
F = @(s)(GammaProd(bm,Bm,s) .* GammaProd(1-an,-An,s).* z.^-s ) ./ ...
(GammaProd(1-bq,-Bq,s) .* GammaProd(ap,Ap,s));

%% Parameters:
p = length([An Ap]);

%q = length([An Ap]); % original
% -------------------------------
q = length([Bm Bq]);
% -------------------------------
...
if(~condition1)
    ...
    out = (1/(2i*pi))*integral(F,-infity,-infity, 'Waypoints',WayPoints);
    return
end
if(condition2)
    infity = 100;
